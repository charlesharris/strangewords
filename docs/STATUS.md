...
| 4 — Pacing / motion / a11y / copy | 🟡 partial | basic cross-fades, Reduce-Motion-aware scene, initial copy, basic VoiceOver labels, **bundled fonts (Fraunces + Inter)**, **softened syllable nudge**, **dissolution effect (swappable; falling petals)**. **Missing:** reveal/compose micro-animations, haptics, Dynamic Type audit, final copy |
...
| Design pass 3 — pixel-art scene | ✅ done | procedural pixel-art day/night: Bayer-dithered sky, pixel sun/moon, twinkling stars, drifting clouds, stepped hills. **Now the active backdrop** (swappable via `RootView.sceneStyle`) |
...
- **Dissolution is a swappable effect.** `Design/Dissolution.swift` defines `DissolutionEffect` + a registry (`Dissolutions.current`); `RevealView` plays whatever's active when you tap "let it go", and transitions only once the effect signals done. Ships with `PetalDissolution` (poem lifts & fades while cherry petals drift down — Reduce-Motion-aware) and a minimal `FadeDissolution` alternate. Add new effects by conforming the protocol and pointing the registry at them; the reveal flow never changes.
...
4. **Pixel-art polish** — a richer night moon, a pixel cherry branch/blossom motif, per-state scene shifts (deepen toward dusk during reveal), a pixel dissolution variant to match the new backdrop, haptics.
5. **Phase 2 push** — unlocks the actual async/durable-connection experience (needs Apple account).
6. **Phase 5 safety + Phase 6 TestFlight** — required before real strangers can use it.

Open richer-scene ideas: water/reflection at the bottom, per-state scene shifts (deepen toward dusk during reveal), a torii/bridge silhouette, smooth theme transitions across an hour boundary.
