...
> **Snapshot:** 2026-06-16
> **Branch:** `build/backend-core` (not yet merged to `main`)
...
| 4 — Pacing / motion / a11y / copy | 🟡 partial | basic cross-fades, Reduce-Motion-aware scene, initial copy, basic VoiceOver labels. **Missing:** real dissolution/reveal animation, haptics, Dynamic Type audit, final copy |
...
| Design pass 2 — richer scene | ✅ done | ridges, celestial body, stars/clouds, cherry branch |
...
- **Fonts:** the boards specify **Larken (display serif) + Inter (body)**; these are **not bundled yet** — the system serif/sans stand in. Bundling them is the highest-leverage remaining "feel" change.
...
1. **Bundle Larken + Inter** — biggest feel change for least effort; typography is the primary design material.
2. **Falling-petals dissolution** — marry the scene to the emotional climax (the dissolution is the app's philosophical center).
3. **Phase 2 push** — unlocks the actual async/durable-connection experience (needs Apple account).
...
On branch `build/backend-core`. Commits so far (newest first):
richer scene → theme flags → time-of-day theming → live_test → robot poet →
run.sh → iOS core loop → backend core + plan/briefs. Plus fixes: token storage,
script hardening (stray backend / Redis-ready), stray-simulator shutdown.

Not yet merged to `main`; no remote configured.
